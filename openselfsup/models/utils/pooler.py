...
def assign_boxes_to_levels_soco(
    box_lists: List[Boxes],
    soco_assign_sizes,
):
    assert  len(soco_assign_sizes) == 4
    box_sizes = torch.sqrt(cat([boxes.area() for boxes in box_lists]))

    level_assignments = box_sizes.new_full((len(box_sizes),), 0, dtype=torch.int64)
    level_assignments[box_sizes <= soco_assign_sizes[0]] = 3
    level_assignments[box_sizes <= soco_assign_sizes[1]] = 2
    level_assignments[box_sizes <= soco_assign_sizes[2]] = 1
    level_assignments[box_sizes <= soco_assign_sizes[3]] = 0
...
class ROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes], soco_assign_sizes=None):
        """
        Args:
        ...
        if not soco_assign_sizes:
            level_assignments = assign_boxes_to_levels(
                box_lists, self.min_level, self.max_level, self.canonical_box_size, self.canonical_level
            )
        else:
            level_assignments = assign_boxes_to_levels_soco(box_lists, soco_assign_sizes)
